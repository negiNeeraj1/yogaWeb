...
export const createPayment = async (req, res) => {
    try {
        ...
        res.status(201).json({ success: true, data: savedPayment, orderId: order.id });
    } catch (error) {
...
export const verifyPayment = async (req, res) => {
    try {
        ...
        if (expectedSignature === razorpay_signature) {
            ...
            const existingRegistration = await ClassAttendance.findOne({ yogaClass: classId, user: userId });
